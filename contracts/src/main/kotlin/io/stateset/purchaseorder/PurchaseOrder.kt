...
data class PurchaseOrder(val purchaseOrderNumber: String,
    ...
    override val participants: List<AbstractParty> get() = listOf(purchaser, vendor)
    ...
    override fun toString(): String {
        ...
        return "PO ($linearId): $vendorString has an received a Purchase Order from $purchaserString for $total and the current status is."
    }
    ...
    override fun verify(tx: LedgerTransaction) {
        ...
        when(purchaseOrderCommand.value) {
            ...
            is Commands.ReviewPurchaseOrder -> requireThat {
                ...
                "only the job status should change" using (purchaseOrderInput.copy(purchaseOrderStatus = purchaseOrderOutput.purchaseOrderStatus) == purchaseOrderOutput)
            ...
            is Commands.CompletePurchaseOrder -> requireThat {
                ...
                "the purchaseOrder in unchanged apart from the status field" using (purchaseOrderInput.copy(purchaseOrderStatus = purchaseOrderOutput.purchaseOrderStatus) == purchaseOrderOutput)
            }
            ...
            is Commands.CancelPurchaseOrder -> requireThat {
                ...
                "only the status must change" using (purchaseOrderInput.copy(purchaseOrderStatus = purchaseOrderOutput.purchaseOrderStatus) == purchaseOrderOutput)
